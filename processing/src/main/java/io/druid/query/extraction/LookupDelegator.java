...
public class LookupDelegator extends LookupExtractor
{
  ...
  @JsonCreator
  public LookupDelegator(@JacksonInject LookupReferencesManager lookupReferencesManager, @JsonProperty("lookupName") String lookupName) {
    this.lookupName = Preconditions.checkNotNull(lookupName, "lookupName can not be null");
    this.lookupReferencesManager = Preconditions.checkNotNull(lookupReferencesManager, "lookup reference manager can not be found");
    this.delegate = this.lookupReferencesManager.get(this.lookupName);
  }
  ...
  private void assertDelegateNotNull()
  {
    if (delegate == null){
      throw new ISE("can not find lookup with name [%s]", lookupName);
    }
  }
