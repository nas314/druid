...
public class LookupExtractionModule implements DruidModule
{
  ...
  public List<? extends Module> getJacksonModules()
  {
    return ImmutableList.<Module>of(
        new SimpleModule("Cached-LoadingOrPolling-Lookup-Module")
        {
          @Override
  ...
  {
    byte[] randomBytes = StringUtils.toUtf8(UUID.randomUUID().toString());
    ByteBuffer byteArray = ByteBuffer.allocate(randomBytes.length);
    return byteArray.put(randomBytes).array();
  }
}
