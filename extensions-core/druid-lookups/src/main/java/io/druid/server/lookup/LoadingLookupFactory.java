...
public class LoadingLookupFactory implements LookupExtractorFactory
{
  ...
  )
  {
    ...
    this.loadingLookup = new LoadingLookup(this.dataFetcher, this.loadingCache, this.reverseLoadingCache);
  }
  ...
  )
  {
    ...
    this.reverseLoadingCache = reverseLoadingCache;
  ...
  public boolean start()
  {
    if (!started.get()) {
      ...
      LOGGER.info("starting loading lookup");
    }
    return started.get();
  }
  ...
  public boolean close()
  {
    if (started.getAndSet(false)) {
      LOGGER.info("closing loading lookup");
      loadingLookup.close();
    }
    return !started.get();
  }
