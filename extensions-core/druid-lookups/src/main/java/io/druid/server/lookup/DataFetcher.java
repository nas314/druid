...
public interface DataFetcher<K, V>
{
   ...
   *
   * @return Returns a map containing All the key-value pairs.
   */
  Map<K, V> fetchAll();
   ...
   *
   * @return Returns a {@link Map} containing only the pair of existing key-value.
   */
  Map<K, V> fetch(Iterable<K> keys);
