...
public class PollingLookupTest
{
  ...
  private static class MockDataFetcher implements DataFetcher
  {
    ...
    public Map fetchAll()
    {
      if (callNumber == 0) {callNumber +=1; return firstLookupMap;}
      return secondLookupMap;
    }
    ...
    @Override
    public Map fetch(Iterable keys)
    {
      return null;
    }
  ...
  {
    //EasyMock.expect(dataFetcher.fetchAll()).andReturn(firstLookupMap).anyTimes();
    //EasyMock.replay(dataFetcher);
    pollingLookup = new PollingLookup(Period.millis(POLL_PERIOD), dataFetcher, pollingCacheFactory);
  }
  ...
  {
    PollingLookup pollingLookup2 = new PollingLookup(Period.millis(1), dataFetcher, pollingCacheFactory);
